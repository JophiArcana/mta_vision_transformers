...
class OpenCLIPNystromCompressionViT(OpenCLIPViT):
    ...
    def __init__(
        ...
        def profile(_C: torch.Tensor):
            print(f"\tSum: {torch.sum(_C).item()}")
            print(f"\tMin: {torch.min(_C).item()}, median: {torch.median(_C).item()}, max: {torch.max(_C).item()}")
                ...
                if compression_condition:
                    ...
                    def get_propagation_query_and_key(
                        ...
                        if self.mode in ["fps", "uniform"]:
                            guarantee_mask = mask_dict.get("guarantee", empty_mask)                 # bool: [bsz x N]
                            exclude_mask = mask_dict.get("exclude", empty_mask)                     # bool: [bsz x N]
                            ...
                            manual_mask = mask_dict.get("manual", empty_mask)                       # bool: [bsz x N]
                    ...
                    explicit_mask = mask_dict.get("explicit", empty_mask)   # bool: [bsz x N]
                    ...
                    def compute_extension(qp: torch.Tensor, kp: torch.Tensor, qi: torch.Tensor, ki: torch.Tensor) -> torch.Tensor:
                        A = torch.softmax(qp @ kp.mT, dim=-1)               # float: [bsz x h x num_sample x num_sample]
                        B = torch.softmax(qp @ ki.mT, dim=-1)               # float: [bsz x h x num_sample x num_implicit]
                        BT = torch.softmax(qi @ kp.mT, dim=-1)              # float: [bsz x h x num_implicit x num_sample]
        ...
        for idx, blk in enumerate(self.model.visual.transformer.resblocks):
            ...
            for handle in self.attention_returns:
                blk.attn.register_module(OpenCLIPNystromCompressionViT.return_module_name(handle), nn.Identity())
