...
# Create a zip file containing the default images
with zipfile.ZipFile('/workspace/default_images.zip', 'w') as zipf:
...
def rgb_from_ncut_discrete_hirarchical(feats, color_num_eig=50, num_clusters=[10, 50, 250], 
    ...
    eigvecs, eigvals = NCUT(num_eig=num_eig, device='cuda:1', move_output_to_cpu=False,
                            affinity_focal_gamma=gamma, distance=distance).fit_transform(feats)
...
plt.suptitle('DiNO Features', fontsize=16)
plt.tight_layout()
plt.show()
